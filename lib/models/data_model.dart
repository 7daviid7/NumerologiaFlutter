...
class DataModel with ChangeNotifier {
  ...
  void _calculateValues() {
    ...
    List<String> primerArc = [
      'NL',
      'Expresión',
      'Apertura',
      'Desarrollar',
    ];
    List<String> segonArc = [
      'NL',
      'Alma',
      'Renacer',
      'Evolutivo',
  ...
  void valorsFigura() {
    ...
    for (int i = 1; i <= 9; i++) {
      ...
      keyValue =
          (keyValue % 9) + 1; // increment and wrap around to 1 if it reaches 10
    }
  }

  // Assegura't que aquesta funció és pública
  Map<String, int> valors(
      Map<String, int> results, List<String> keysToTransfer) {
    Map<String, int> valors = {};
  ...
  Map<String, dynamic> toJson() {
    return {
      'name': name,
      'date': date,
      'mapPersonalidad': mapPersonalidad,
      'mapVida': mapVida,
      'mapDesafio': mapDesafio,
      'mapHerencies': mapHerencies,
      'mapPrimerArc': mapPrimerArc,
      'mapSegonArc': mapSegonArc,
      // 'taula': taula.toString(), // Complex object, simplifying for now if needed or keep as is if JSON encodable
      'habitants':
          habitants.map((key, value) => MapEntry(key.toString(), value)),
      'reduceLife': reduceLife,
      'any': any,
      'mes': mes,
      'dia': dia,
      'mapFigura':
          mapFigura.map((key, value) => MapEntry(key.toString(), value)),
      'yin': yin,
      'yang': yang,
    };
  }
